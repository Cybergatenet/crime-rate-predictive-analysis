...
from wtforms import EmailField, DateTimeField
...
connection = DB.connect("crime_DB")

mycursor = connection.cursor()

# cursor.execute("CREATE TABLE IF NOT EXISTS crimes ( crime_id INT AUTO_INCREMENT PRIMARY KEY, crime_type VARCHAR(255) NOT NULL, crime_status INT DEFAULT 1)")

try:
    with open('C:\\PYTHON PROJECT\\Crime-Rate-Prediction\\crime_ copy.sql', 'r') as f:
        sql_scripts = f.read()

    # Split the script into individual commands
    sql_commands = sql_scripts.split(';')
...
app.config['MAIL_USERNAME']='onwudiegwuphil@gmail.com'
app.config['MAIL_PASSWORD']='chilz111'
...
def contact():
    ...
    if request.method == 'POST':
        ...
        try:
            cursor.execute("Insert into contact(name,email,subject,messaage,date_feedback) values(%s,%s,%s,%s,%s)",[name,email,subject,message,date])
            connection.commit()
...
class RegistrationForm(Form):
    ...
    mobile=StringField('Mobile',[validators.DataRequired("Please Enter your Mobile Number"),validators.Regexp(regex=r'\d{11}$',message="Mobile Number must be of 10 Digits")])
...
def register():
    ...
    if request.method == 'POST' and form.validate():
        ...
        date_register = datetime.utcnow().strftime('%s')
        ip = request.environ.get('HTTP_X_REAL_IP', request.remote_addr)

        print("Data to be inserted:", name, email, mobile, addhar, password, date_register, ip)

        # creating cursor
        cursor = connection.cursor()

        try:
            # checking if email or addhar already exist
            result = cursor.execute("SELECT email, aadhar FROM users WHERE email = %s OR aadhar = %s", (email, addhar))
            if result > 0:
                data = cursor.fetchone()
                if data['email'] == email:
                    flash('Email already exists...!', 'danger')
                elif data['aadhar'] == addhar:
                    flash('Aadhar already exists..!', 'danger')
            else:
                # inserting data
                cursor.execute("INSERT INTO users(name, email, mobile, aadhar, password, date_register, ip_address) VALUES(%s, %s, %s, %s, %s, %s, %s)",
                               (name, email, mobile, addhar, password, date_register, ip))

                # committing inserted data
                connection.commit()

                # Send verification email (similar to your existing logic)
                print("Data inserted:", name, email, mobile, addhar, password, date_register, ip)

                flash('Registration successful!', 'success')
                print("Redirecting to register_verify page...")
...
def login():
    if request.method =='POST':
        ...
        if result > 0:
            ...
            data=cursor.fetchone()
            passoword_db = data['password']

            if data['status'] != 0:
                #comparing passwords
                if sha256_crypt.verify(password_candidate,passoword_db):
...
def add_crime():
    if request.method == 'POST':
        ...
        if result == 0:
            #executing the query to fetch the user data
            res=cursor.execute("INSERT INTO crimes(crime_type) VALUES(%s)",[crime_type])
...
@app.route('/delete_crime/<crime_id>')
@login_required
...
@app.route('/restore_crime/<crime_id>')
@login_required
